  ...
  useEffect(() => {
    if (workout.pdfUrl) {
      ...
      fetch(workout.pdfUrl)
        ...
        .then(text => {
          // Parse CSV
          // Suporta tanto CSV com separação por vírgula quanto por ponto-e-vírgula
        ...
        {csvData && !isLoading && (
          <div className="p-4">
                ...
                <TableHeader>
                  <TableRow className="bg-muted/50">
                    {csvData.headers.map((header, index) => (
                      <TableHead key={index} className="font-bold text-center">{header}</TableHead>
                    ))}
                  </TableRow>
                </TableHeader>
                <TableBody>
            ...
            <p className="text-xs text-muted-foreground mt-4 text-right">
              Total de linhas: {csvData.rows.length}
            </p>
          </div>
        )}
      </div>
